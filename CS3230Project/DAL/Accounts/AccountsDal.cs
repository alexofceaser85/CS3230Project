...
namespace CS3230Project.DAL.Accounts
{
    ...
    public static class AccountsDAL
    {
        ...
        public static void Login(string userName, string password)
        {
            if (userName == null)
            {
                throw new ArgumentException(AccountsDalErrorMessages.LoginUserNameCannotBeNull);
            }

            if (userName.Trim().Length == 0)
            {
                throw new ArgumentException(AccountsDalErrorMessages.LoginUserNameCannotBeEmpty);
            }

            if (password == null)
            {
                throw new ArgumentException(AccountsDalErrorMessages.LoginPasswordCannotBeNull);
            }

            if (password.Trim().Length == 0)
            {
                throw new ArgumentException(AccountsDalErrorMessages.LoginPasswordCannotBeEmpty);
            }
